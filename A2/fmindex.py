...
def naiveOcc(bw):
    ''' Given BWT string bw, return the occurrence array Occ[i][a]
        ...
        i is the number of characters. tots is the tally.  '''
    tots = {}
    for c in bw:
        tots[c] = 0
    ranks = []
...
def divisibleOcc(F):
    ''' Down sample the first column of the suffix array in order to keep only the
     128th suffix, and its multiples.'''
    dF = {}
...
def traceback(m, n, occ, bw, a):
    r = m % 128 #remainder
    start = m - r

    if r < 64:
        n_occ = occ[start/128][a]
        for i in range(start + 1, n + 1):
            if bw[i] == a:
                n_occ += 1
    else:
        n_occ = occ[start/128 +1][a]
        for i in range(start + 128 - 1, n - 1, -1):
            if bw[i+1] == a:
                n_occ -= 1
    return n_occ


def count(S, occ, C, bw):
    # S is count, Occ is Ranks, C is F
    ...
    while i >= 0:
        ...
        l = C[a][0] + traceback(l, l-1, occ, bw, a)

        u = C[a][0] + traceback(u, u, occ, bw, a) - 1
...
print "%s occurs %d times" % (args.count, count(args.count, ranks, F, bwt))
